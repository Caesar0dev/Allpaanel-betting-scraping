            ...
            if (url === 'https://www.allpaanel.com/api/user/gamehighlight') {
                ...
                try {
                    ...
                    for (let i = 0; i < decryptedData.length; i++) {
                        ...
                        let apiTag = null;
                        let dataToServer = {};
                        Object.assign(dataToServer, { id: matchID, marketId: dataID })
                        let data;
                
                        if (vir == 1) {
                            matchURL = `https://www.allpaanel.com/game-detail/${matchID}`;
                            apiTag = '/api/game/getdata';
                            data = await page.evaluate(async (matchID, dataID, apiTag, matchURL, x_csrf_token, x_xsrf_token) => {
                                // defined body
                            ...
                            Object.assign(dataToServer, data)
                        } else {
                            matchURL = `https://www.allpaanel.com/casino/cricketvirtual/${matchID}`;
                            apiTag = '/api/cricketv/data';
                            data = await page.evaluate(async (matchID, dataID, apiTag, matchURL, x_csrf_token, x_xsrf_token, matchType) => {
                                // defined body
                        ...
                        fullMatch.push(dataToServer)
                  
                        //     const cricketServerURL = `http://localhost:5000/cricketdata`;
                        //   await sendToDBServer(cricketServerURL, dataToServer);
                  
                    }
                    const cricketServerURL = `https://scraper-backend-yg6r.onrender.com/cricketdata`;
                    await sendToDBServer(cricketServerURL, fullMatch);
                    resolve(fullMatch)
                } catch (error) {
